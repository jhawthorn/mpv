...
int mpcodecs_reconfig_vo(sh_video_t *sh, struct mp_image *prototype)
{
    struct MPOpts *opts = sh->opts;
    vf_instance_t *vf = sh->vfilter;
    int vocfg_flags = 0;
    struct mp_image pt = *prototype;

    if (!pt.w || !pt.h) {
        // ideally, this should be dead code
        mp_msg(MSGT_DECVIDEO, MSGL_WARN, "Uknown size, using container size.\n");
        mp_image_set_size(&pt, sh->disp_w, sh->disp_h);
    } else {
        sh->disp_w = pt.w;
        sh->disp_h = pt.h;
    }
    ...
    mp_msg(MSGT_DECVIDEO, MSGL_V, "VDec: vo config request - %d x %d (%s)\n",
           pt.w, pt.h, vo_format_name(pt.imgfmt));
    ...
    for (;;) {
        ...
        flags = vf->query_format(vf, pt.imgfmt);
        mp_msg(MSGT_CPLAYER, MSGL_DBG2, "vo_debug: query(%s) returned 0x%X \n",
               vo_format_name(pt.imgfmt), flags);
    ...
    mp_image_set_display_size(&pt, d_w, d_h);
    ...
    mp_msg(MSGT_CPLAYER, MSGL_V,
           ...
           sh->disp_h, d_w, d_h, vocfg_flags, pt.imgfmt);

    if (vf_reconfig_wrapper(vf, &pt, vocfg_flags) < 0) {
    ...
    sh->colorspace = pt.colorspace;
    sh->color_range = pt.levels;
